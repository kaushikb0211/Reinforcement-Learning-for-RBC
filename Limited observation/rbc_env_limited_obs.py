...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Set x and z observation ranges for RBC environment.")
    parser.add_argument("--x_start", type=float, default=0.0, help="Start of the x observation range (fraction of Lx)")
    parser.add_argument("--x_end", type=float, default=1.0, help="End of the x observation range (fraction of Lx)")
    parser.add_argument("--z_start", type=float, default=0.0, help="Start of the z observation range (fraction of Lz)")
    parser.add_argument("--z_end", type=float, default=1.0, help="End of the z observation range (fraction of Lz)")
...
class DedalusRBC_Env(gym.Env):
    ...
    def __init__(self,
        ...
        self.obs_metadata = {
            "Ni": 30,  # Can also be made dynamic if needed
            "Nk": 8
        }
        ...
        print(f"Initializing DedalusRBC_Env with x_start={self.x_start}, x_end={self.x_end}, z_start={self.z_start}, z_end={self.z_end}")
    ...
    def _extractObs(self):
        ...
        x = np.linspace(
            self.sim_metadata['Lx'] * self.x_start, 
            self.sim_metadata['Lx'] * self.x_end, 
            self.obs_metadata['Ni'] + 1
        )[:-1]  # Remove last point due to periodicity

        z = np.linspace(
            0.1 + self.sim_metadata['Lz'] * self.z_start, 
            self.sim_metadata['Lz'] * self.z_end - 0.1, 
            self.obs_metadata['Nk']
        )
